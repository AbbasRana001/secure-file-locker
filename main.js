...
function renderOutput(text, mode) {
  outputText = text;
  // Hide placeholder, switch box to normal block layout for real content
...
/* ── Tab / page navigation ──
   Handles both the nav tabs AND any button with data-page="..."
   (e.g. the "Try it yourself" CTA button on the About page).    */
function switchPage(target) {
  // Mark the correct nav tab as active
  document.querySelectorAll(".nav-tab").forEach((t) => {
    t.setAttribute("aria-selected", t.dataset.page === target);
  });
  // Show the matching page, hide all others
  document.querySelectorAll(".page").forEach((p) => {
    p.hidden = p.id !== `page-${target}`;
  });
  // Scroll to top so user sees the page from the beginning
  window.scrollTo({ top: 0, behavior: "smooth" });
}

// Attach to every element that has a data-page attribute
